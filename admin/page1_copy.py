...
class Page1Test(QMainWindow):
    def __init__(self):
        ...
        self.vr = 0
        self.tong_xe = 3
        self.so_xe = self.get_available_parking_lot()["count"]
        print("con lai = ",self.con_lai)
        self.con_lai = self.tong_xe - self.so_xe
    ...
    def Time(self):
        ...
        try:
            ...
            if self.cp == 1:
                if self.vr == 1:
                    user_detail = self.check_card_id_in_user(self.cardId)
                    ...
                    if self.so_xe < self.tong_xe:
                        ...
                        if user_detail and user_detail["licensePlateNumber"] == self.data:
                            ...
                            self.ser.flush()
                        elif user_detail and user_detail["licensePlateNumber"] != self.data:
                            status = "NOT_OK"
                            self.ser.write(f'd|{status}\r\n'.encode())
                            self.ser.flush()
                            msg_box = QMessageBox()
                            msg_box.setWindowTitle("Thông báo")
                            msg_box.setText("Thẻ không hợp lệ!")
                            ...
                            msg_box = QMessageBox()
                            msg_box.setWindowTitle("Thông báo")
                            msg_box.setText("Thẻ đã được sử dụng cho khách hàng khách!")
                            msg_box.setIcon(QMessageBox.Warning)
                            msg_box.setStandardButtons(QMessageBox.Ok)  # Hiển thị nút OK
                            msg_box.exec_()
                            ...
                            self.ser.write(f'd|{status}\r\n'.encode())
                            self.ser.flush()
                            create_user_transient = self.create_user_transient(self.cardId, self.data)
                            self.con_lai = self.con_lai - 1
                            self.so_xe = self.so_xe + 1
                ...
                if self.vr == 2:
                    ...
                    if user_detail and user_detail["licensePlateNumber"] == self.data:
                        ...
                        status = "OK"
                        data = self.handle_check_out(self.cardId)
                        plate_license_number = data["plate_license_number"]
                        ...
                        time_part, date_part = time_present.split(" - ")
                        datetime_out = datetime.strptime(f"{date_part} {time_part}", "%d/%m/%Y %H:%M:%S")
                        datetime_in = datetime_out - timedelta(seconds=time_seconds)
                        time_out_str = datetime_out.strftime("%H:%M:%S - %d/%m/%Y")
                        time_in_str = datetime_in.strftime("%H:%M:%S - %d/%m/%Y")
                        self.lb_bien_vao.setText()
                        self.lb_tgian_ra.setText(time_present)
                        self.lb_tgian_gui.setText(str(time_seconds) + " s")
    ...
    def check_card_id_in_user(self, rfid_value):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get("code") == 1000:
                    ...
                    print(f"Error: {data.get('message')}")
                    return {"count": 0}  # Giá trị mặc định
            else:
                print(f"HTTP Error: {response.status_code}")
                return {"count": 0}  # Giá trị mặc định
        except Exception as e:
    ...
    def create_user_transient(self, card_id, plate_license_number):
        url = "http://localhost:8080/smart-parking/users-transient/create-user-transient"
        headers = {
            "Content-Type": "application/json"
        }
        payload = {
            "cardId": card_id,
            "plateLicenseNumber": plate_license_number,
        }
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get("code") == 1000:
                    result = data["result"]
                    print(f"User Transient Created: {result}")
                    return result  # Trả về thông tin người dùng vừa tạo
                else:
                    print(f"Error: {data.get('message')}")
                    return None
            else:
    ...
    def handle_check_out(self, card_id):
            url = "http://localhost:8080/smart-parking/users-transient/find-user-transient-by-card-id"
            headers = {
                "Content-Type": "application/json"
            }
            payload = {
